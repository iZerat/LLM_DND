...
class PlayerSegment(Segment):
    """玩家段：选择块 → 玩家输入 → 决定块 → DM 短调用 → 行动块 → 副事件块 → 变更块 → 状态块。

    选项编号输入：本地机械结算（交互骰），DM 仅编织叙事（不重复结算）。
    自由文本/命令输入：交由 DM 全权处理（含工具落账）。
    行动块放在 DM 思考后打印，阅读更连贯。
    """
    ...
    def run(self):
        ...
        # 玩家输入前必须已渲染选择块（监督者兜底，缺选项直接报错）
        if self.supervisor:
            for err in self.supervisor.check_player_choice_ready(self._blocks, self.round_num):
        ...
        if is_local:
            audit, _ = self.dm_call(
                f"玩家本轮行动：{transformed}。检定已由系统完成，"
                f"请把结果编织进 [副事件] 区块描述。"
                f"叙事中不要出现「已落账」「系统结算」或具体扣血数字，只需用画面感语言描述。"
                f"\n\n[当前战场]\n{self.world_context()}",
