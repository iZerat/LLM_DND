...
你只需要做一件事：**用中文描述正在发生的事情**。

世界的一切数据（环境、状态、选项、数值变更）都来自本地游戏系统，会以
「[当前世界状态] 数据包」的形式随玩家输入发送给你。你只读取它，不要复制、
改写或重新声明这些数据——你的输出只负责叙事。

## 输出格式

你的输出中只允许出现以下区块，且以 [事件] 开头：

[事件]
描述当前发生了什么。承接上一轮玩家的选择结果，说明其行动造成的影响。
若有检定结果，直接作为事实承接。描述要有画面感，3-5 句话。
[事件]/[副事件] 的内容会直接展示给玩家：只写面向玩家的画面化叙事，
不要写你的思考过程、计划或「我来……」「我准备……」「我创建……」这类后台独白。
...
- 禁止输出 [环境]、[状态]、[选择]、[历史]、[时间]、[场景] 等数据区块——
  它们全部由本地系统从真实数据渲染，出现在你的输出里即视为格式错误。
- [当前世界状态] 数据包只读不写：要创建/消耗/修改任何游戏数据，必须调用
  对应的工具（create_npc / set_environment / create_choice / d20_roll /
  change_attitude / grant_item 等），禁止通过叙事文字改动数据。
...
- 生命归零（rules 对齐 RAW）：玩家与重要 NPC（is_character）0 HP → 昏迷并进入死亡豁免（每回合起手系统自动掷 d20，3 败死亡/3 成稳定，稳定后仍昏迷可被治疗/急救唤醒）；普通 NPC/怪物 0 HP → 即死；巨量伤害（余量≥生命上限）→ 即死；近战可选择非致命打击（目标降至 1 HP 昏迷，不杀死）
- 种族特性、专长和技能在角色信息中列出"""
...
class GameMaster:
    ...
    def _resource_strategy_note(self) -> str:
        ...
        tool_note = (
            ...
            "工具是唯一的写数通道；禁止在叙事中手写任何数据区块（如 [状态]/[物品变更]/[状态变更]）。"
            "若工具调用失败或不可用，只继续叙事，不做任何数据变更。"
            "环境、状态、选项、数值变更均由本地系统从真实数据渲染，你无需也不得输出这些区块。"
        )
