...
- set_environment — 每轮更新当前场景环境（地点/温度/天气/氛围等场景字段）与世界时间（年月日/季节/时分秒/时段等时间字段）
...
class GameMaster:
    ...
    def _resource_strategy_note(self) -> str:
        ...
        tool_note = (
            ...
            "工具是唯一的写数通道，不要在叙事中手写任何数据。"
            "若工具调用失败或不可用，只继续叙事，不做任何数据变更。"
            "环境、状态、选项、数值变更均由本地系统从真实数据渲染，你只需输出叙事文本。"
        )
